import sys
# DEBUG: Top of file loaded
print("DEBUG: ser_gui_pyqt5.py loaded")
"""
...
class SERGUI(QMainWindow):
    def _analyze_emotion(self):
        """Start emotion analysis in a worker thread."""
        if not hasattr(self, 'current_file') or not self.current_file:
            QMessageBox.warning(self, "No File", "Please select or record an audio file first.")
            return
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.worker = None  # Ensure worker attribute always exists
        self.prediction_history = []  # Fix: ensure attribute exists
        self.max_history = 30  # Fix: ensure attribute exists
        # If you have other initialization, call it here, e.g.:
        self._build_ui()
    def _record_audio(self):
        ...
        self.emotion_label.setText("Recording...")
        ...
        def do_record():
            try:
                ...
                QMessageBox.critical(self, "Recording Error", f"Failed to record audio: {str(e)}")
            finally:
                self.record_btn.setEnabled(True)
        threading.Thread(target=do_record).start()
    def _reset_display(self):
        self.results_section.hide()
        self.emotion_label.setText("Select a file to analyze")
        self.confidence_label.setText("")
        self.circular_progress.setValue(0)
        self._set_emotion_svg('neutral')
    def _build_ui(self):
    ...
    def _history_section(self):
        ...
        frame = QFrame()
        frame.setStyleSheet("QFrame { background-color: #F3F4F6; border-radius: 8px; padding: 4px 8px 4px 8px; border: 1px solid #E5E7EB; margin-top: 4px; }")
        v = QVBoxLayout(frame)
        v.setContentsMargins(4, 4, 4, 4)
        v.setSpacing(2)

        h = QHBoxLayout()
        lbl = QLabel("Prediction History (last 30)")
        lbl.setStyleSheet("QLabel { font-size: 11px; font-weight: bold; color: #2C3E50; }")
        h.addWidget(lbl)
        h.addStretch(1)
        ...
        self.history_table.setHorizontalHeaderLabels(["Timestamp", "File", "Emotion", "Confidence"])
        ...
        self.history_table.setFixedHeight(90)
        self.history_table.setStyleSheet("""
            QTableWidget { background: white; font-size: 10px; border-radius: 4px; }
            QHeaderView::section { font-weight: bold; background: #E5E7EB; color: #22223B; border: none; font-size: 10px; }
            QTableWidget::item { padding: 1px; }
            QTableWidget::item:selected { background: #B8D4F0; }
    ...
    def _update_history(self, file, emotion, confidence):
        import datetime
        from PyQt5.QtWidgets import QTableWidgetItem
        ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        entry = {"timestamp": ts, "file": file, "emotion": emotion, "confidence": confidence}
    ...
    def _refresh_history_table(self):
        ...
        for i, entry in enumerate(self.prediction_history):
            ...
            self.history_table.setItem(i, 1, QTableWidgetItem(entry["file"]))
            self.history_table.setItem(i, 2, QTableWidgetItem(entry["emotion"]))
            self.history_table.setItem(i, 3, QTableWidgetItem(f"{entry['confidence']*100:.1f}%"))
        self.history_table.scrollToBottom()

    def _export_history(self):
        import csv
        from PyQt5.QtWidgets import QFileDialog
        path, _ = QFileDialog.getSaveFileName(self, "Export Prediction History", "prediction_history.csv", "CSV Files (*.csv)")
        if not path:
            return
        try:
            with open(path, "w", newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["Timestamp", "File", "Emotion", "Confidence"])
                for entry in self.prediction_history:
                    writer.writerow([entry["timestamp"], entry["file"], entry["emotion"], f"{entry['confidence']*100:.1f}%"])
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.information(self, "Export Complete", f"Prediction history exported to:\n{path}")
        except Exception as e:
            from PyQt5.QtWidgets import QMessageBox
    ...
    def _build_ui(self):
        ...
        try:
            ...
            main.addWidget(status)
            print("_build_ui: status added")

            print("_build_ui: adding file section")
            main.addWidget(self._file_section())
            print("_build_ui: file section added")

            print("_build_ui: adding emotion section")
            main.addWidget(self._emotion_section())
            print("_build_ui: emotion section added")

            print("_build_ui: adding results section")
            self.results_section = self._results_section()
            self.results_section.hide()
            main.addWidget(self.results_section)
            print("_build_ui: results section added")

            print("_build_ui: adding requirements section")
            main.addWidget(self._requirements_section())
            print("_build_ui: requirements section added")

            print("_build_ui: adding history section")
            main.addWidget(self._history_section())
            print("_build_ui: history section added")
        except Exception as e:
            print(f"_build_ui: Exception occurred: {e}")
            import traceback
            traceback.print_exc()
        print("_build_ui: finished")
    ...
    def _on_finished(self, results: dict):
        try:
            ...
            emotion = results.get('overall_emotion', '').lower()
            confidence = float(results.get('confidence', 0.0))
            self._update_history(file, emotion, confidence)
            # Alert for negative emotions
            if emotion in ('anger', 'angry', 'fear', 'fearful'):
                QMessageBox.warning(self, "Negative Emotion Detected", f"Alert: Negative emotion '{emotion}' detected!")
        except Exception as e:
