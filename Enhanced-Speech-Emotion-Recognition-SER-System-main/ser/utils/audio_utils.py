...
def read_audio_file(file_path: str) -> tuple[ndarray, Any]:
    ...
    for _ in range(Config.AUDIO_READ_CONFIG["max_retries"]):
        # Prefer soundfile on Windows for reliability
        try:
            ...
            if audiofile.ndim > 1:
                audiofile = np.mean(audiofile, axis=1)
            max_abs_value: Any = np.max(np.abs(audiofile))
            audiofile = audiofile if max_abs_value == 0 else audiofile / max_abs_value
            ...
            logger.warning(msg=f"Soundfile failed: {err_sf}; trying librosa")
            try:
                ...
                return audiofile, current_sample_rate
            except Exception as err_lb:
                logger.warning(msg=f"Librosa also failed: {err_lb}")
                logger.info(msg=(
